...
@section('content')
    ...
    <x-page-header title="Job Position List">
        <x-slot:actions>
            <form action="{{ route('job-positions.index') }}" method="GET" enctype="multipart/form-data"
                style="width: 200px;">
                @csrf
            ...
            <div class="qpopup-menu">
                <button class="qbtn qpopup-menu-btn">
                    {{ $jobLevels->firstWhere('id', request('job_level_id'))?->name ?? 'All' }}
                    <i class="qml-2 fa-solid fa-caret-down"></i>
                </button>

                <ul class="qpopup-menu-container">
                    <li class="qpopup-menu-item">
                        <a href="{{ route('job-positions.index') }}" class="qpopup-menu-link">
                            All
                        </a>
                    </li>
                    ...
                    @foreach ($jobLevels as $jobLevel)
                        <li class="qpopup-menu-item">
                            <a href="{{ route('job-positions.index', ['job_level_id' => $jobLevel->id]) }}"
                                class="qpopup-menu-link">
                                {{ $jobLevel->name }}
                            </a>
                        </li>
                    @endforeach
            ...
            @can('lookup-value-create')
                <a href="{{ route('job-positions.create') }}" class="qbtn qbtn-blue">
                    Add New
                </a>
            @endcan
        </x-slot:actions>
    ...
    <ul class="qlist-view flex-grow-1 overflow-y-auto">
        @foreach ($data as $item)
            <li class="qlist-view-item">
                <a href="{{ auth()->user()->can('lookup-value-edit') ? route('job-positions.edit', $item->id) : route('job-positions.show', $item->id) }}"
                    class="qlist-tile">
...
{{-- 
@section('modals')
    <x-filter-modal id="filterModal" :action="route('job-positions.index')">
